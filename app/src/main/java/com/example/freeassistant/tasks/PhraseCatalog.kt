...
object PhraseCatalog {
    ...
    fun findCommand(input: String, language: String): String? {
        ...
        generalPhrases[language]?.any { normalized.contains(it) }?.let {
            return if (language == "ru") "помощь" else "help"
        }
        
        // Check open app phrases
        openAppPhrases[language]?.any { normalized.contains(it) }?.let {
            return if (language == "ru") "открыть приложение" else "open"
        }
        
        // Check photo phrases
        photoPhrases[language]?.any { normalized.contains(it) }?.let {
            return if (language == "ru") "индексировать мои фото" else "index my photos"
        }
        
        // Check notes phrases
        notesPhrases[language]?.any { normalized.contains(it) }?.let {
